"""Admit the predeclared third Figshare generalization pair through exact ranges.

The pair was selected from central-directory metadata in PR #52 before any
...
ADL_NEGATIVE = PinnedMember(
    role="negative",
    name="VideoDataset/ADL/SBJ_02_LOC5/ACT16_R_1/20240923151305.mp4",
    compressed_size=949_755,
    uncompressed_size=950_297,
    crc32=0x39C30723,
    local_header_offset=1_232_209_160,
)
FALL_POSITIVE = PinnedMember(
    role="positive",
    name="VideoDataset/Fall/SBJ_09_LOC3/ACT11_R_1/20240915181458.mp4",
    compressed_size=386_050,
    uncompressed_size=386_799,
    crc32=0x7033ABE3,
    local_header_offset=2_371_833_171,
)
...
def select_pinned_pair(members: tuple[ZipMember, ...]) -> tuple[tuple[PinnedMember, ZipMember], ...]:
    ...
    for expected in PINNED_PAIR:
        ...
        if len(matches) != 1:
            raise RuntimeError("third Figshare member identity is missing or changed")
        member = matches[0]
        if not member.is_bounded_video:
            raise RuntimeError("third Figshare member is outside the reviewed extraction envelope")
...
def admit_pinned_pair(output_dir: str | Path, *, opener: Any) -> tuple[AdmittedMember, ...]:
    ...
    for expected, member in selected:
        ...
        filename = (
            "figshare-g3-adl-negative.mp4"
            if expected.role == "negative"
            else "figshare-g3-fall-positive.mp4"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("figshare_generalization3_member_admission=" + payload)
    if os.getenv("GITHUB_ACTIONS") == "true":
        print("::notice title=Figshare third-pair admission::" + _github_escape(payload))
    return 0
