"""Run the retained person-down path on the third disjoint Figshare pair.

The source supplies clip-level Fall/ADL activity classes, not an independently
published frame-level event interval for these exact clips. The difficult ACT16
Sitting clip is scored as a negative. The ACT11 Try-to-sit-on-chair Fall clip is
retained as diagnostic-only for perception, temporal continuity, candidate
behavior and resource cost; match/miss and alert latency are not fabricated from
a directory label. No detector, association, posture or temporal threshold is
changed.
"""
...
from .figshare_generalization3_admission import admit_pinned_pair
...
_EXPECTED_SHA256: dict[str, str] = {
    "negative": "3e09aa63d79a2cc3398a72ee70ea5a975f9265c29575a17af484548c95df21ac",
    "positive": "a45ea783da1c1312880286882077c7e960bfc3830fcadf7d0b6c5a3aafe40b20",
}
_ACTIVITY = {
    "negative": {
        "activity_code": "ACT16",
    ...
    "positive": {
        "activity_code": "ACT11",
        "activity_name": "Try to sit on chair, fall",
...
def _sample_spec(item) -> ValidationSampleSpec:
    ...
    return ValidationSampleSpec(
        sample_id=f"figshare-g3-{activity['activity_code'].lower()}-{item.role}",
        site_id=f"figshare-28596332-location-{activity['location_id']}",
        camera_id=f"figshare-g3-{activity['activity_code'].lower()}-clip",
...
def run_figshare_generalization3_person_down_diagnostic(
    output_dir: str | Path,
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "diagnostic": "figshare_person_down_generalization_3",
        "pair_id": "generalization-3",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = run_figshare_generalization3_person_down_diagnostic(args.output_dir, args.candidate_dir)
        encoded = json.dumps(payload, allow_nan=False, sort_keys=True, separators=(",", ":"))
        print(encoded)
        import os
        if os.getenv("GITHUB_ACTIONS") == "true":
            print("::notice title=Figshare third-pair person-down diagnostic::" + _github_escape(encoded))
    except (OSError, ValueError, TypeError, RuntimeError, OverflowError):
        print("Figshare third-pair person-down diagnostic rejected.", file=sys.stderr)
        return 2
    return 0
