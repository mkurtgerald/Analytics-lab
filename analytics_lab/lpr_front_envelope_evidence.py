"""Bounded CC0 front-envelope smoke for LPR detector replacement evidence.
...
reviewed OMZ detector's documented 96-pixel minimum, and the photographer
released the work under CC0. The source identity remains fail-closed. This run
compares the already-preserved OMZ miss with one frozen model-free OpenCV
proposal fallback on the exact same decoded image. Neither result is commercial
accuracy evidence.
"""
...
from .lpr_plate_proposals import OpenCVPlateProposalDetector
...
def validate_source_bytes(
    ...
    *,
    ...
    if required_sha1 is not None:
        if not isinstance(required_sha1, str) or len(required_sha1) != 40 or required_sha1.lower() != required_sha1:
    ...
    if required_sha256 is not None:
        if not isinstance(required_sha256, str) or len(required_sha256) != 64 or required_sha256.lower() != required_sha256:
...
def _download_model_files(root: Path) -> None:
    for relative_path, url, size_bytes, sha384 in _MODEL_FILES:
        payload = _fetch_bytes(url, max_bytes=size_bytes, expected_host="storage.openvinotoolkit.org")
        if len(payload) != size_bytes or hashlib.sha384(payload).hexdigest() != sha384:
            raise RuntimeError("OMZ artifact identity mismatch")
        destination = root / relative_path
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_bytes(payload)


def _timed(detector: object, image: object) -> dict[str, object]:
    wall_start = time.perf_counter()
    cpu_start = time.process_time()
    detections = detector(image)  # type: ignore[operator]
    cpu_seconds = time.process_time() - cpu_start
    elapsed_seconds = time.perf_counter() - wall_start
    best = detections[0] if detections else None
    width_fraction = (best.x2 - best.x1) if best is not None else None
    height_fraction = (best.y2 - best.y1) if best is not None else None
    aspect_ratio = (width_fraction / height_fraction) if best is not None and height_fraction else None
    return {
        "plate_detections": len(detections),
        "best_plate_confidence": float(best.confidence) if best is not None else None,
        "best_width_fraction": float(width_fraction) if width_fraction is not None else None,
        "best_height_fraction": float(height_fraction) if height_fraction is not None else None,
        "best_aspect_ratio": float(aspect_ratio) if aspect_ratio is not None else None,
...
def run() -> dict[str, object]:
    source = _fetch_bytes(_SOURCE_URL, max_bytes=_MAX_SOURCE_BYTES, expected_host="upload.wikimedia.org")
    ...
    common: dict[str, object] = {
        ...
        "within_documented_plate_size_envelope": _PREDECLARED_PLATE_WIDTH_LOWER_BOUND_PX >= _DOCUMENTED_MIN_PLATE_WIDTH_PX,
    }
    ...
    try:
        ...
        raise RuntimeError("OpenCV and NumPy are required for LPR front-envelope evidence") from exc
    ...
    proposal_detector = OpenCVPlateProposalDetector()
    proposal_metrics = _timed(proposal_detector, image)

    with tempfile.TemporaryDirectory(prefix="analytics-lpr-front-envelope-") as directory:
        root = Path(directory)
        _download_model_files(root)
        omz_detector = OpenVINOOMZPlateDetector(root)
        omz_metrics = _timed(omz_detector, image)

    return {
        **common,
        "admission_only": False,
        "inference_run": True,
        "omz_runtime_version": omz_detector.runtime_version,
        "omz": omz_metrics,
        "opencv_proposal_runtime_version": proposal_detector.runtime_version,
        "opencv_proposal": proposal_metrics,
        "comparison_note": "proposal score is a frozen heuristic engineering score, not probability or accuracy",
    }
