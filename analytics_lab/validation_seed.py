...
GMDCSA24_ACCEPTED_SEED = (
    SeedMediaSpec("gmdcsa24-s1-fall-05", "Subject 1/Fall/05.mp4", 5_872_655, "4e13ed24f6c2af7062b64b1920ef706c51efe94b", 5_000, 1_800, 5_000, "gmdcsa24-s1-fall-05-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s1-adl-15", "Subject 1/ADL/15.mp4", 7_233_079, "903da9245132cf70c10124edd0625c958f702cb8", 7_000),
)

GMDCSA24_HELDOUT_S2 = (
    SeedMediaSpec("gmdcsa24-s2-fall-01", "Subject 2/Fall/01.mp4", 6_472_725, "6f308a3873d5768ea81b932d45a521e520368592", 6_000, 1_400, 6_000, "gmdcsa24-s2-fall-01-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s2-adl-12", "Subject 2/ADL/12.mp4", 7_791_870, "aa83cc15559687e49c40afad920949be7befa61d", 7_000),
)

GMDCSA24_HELDOUT_S3 = (
    SeedMediaSpec("gmdcsa24-s3-fall-03", "Subject 3/Fall/03.mp4", 5_753_609, "41daaee74ceb00e8db32f34525a57c704994f0b5", 5_000, 1_500, 5_000, "gmdcsa24-s3-fall-03-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s3-adl-08", "Subject 3/ADL/08.mp4", 5_172_948, "a38dca4aed2a27d930354beeb7d36911a7b8dd55", 4_000),
)

GMDCSA24_HELDOUT_S4 = (
    SeedMediaSpec("gmdcsa24-s4-fall-03", "Subject 4/Fall/03.mp4", 2_421_154, "3076cb2d10c3effc103fc501434ed97ebd27ec75", 6_000, 1_700, 6_000, "gmdcsa24-s4-fall-03-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s4-adl-07", "Subject 4/ADL/07.mp4", 5_575_628, "8a6aad020ff6f70b01eb3ecd952b5ec445e0a90d", 4_000),
)

GMDCSA24_ROBUSTNESS_S1_BW = (
    SeedMediaSpec("gmdcsa24-s1-fall-11", "Subject 1/Fall/11.mp4", 5_961_144, "791707e0f4fd062796eb147e55f33eb80455ba38", 6_000, 1_000, 6_000, "gmdcsa24-s1-fall-11-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s1-adl-16", "Subject 1/ADL/16.mp4", 3_582_225, "0991a3657ee6458f470a9bc419d901f192b43d7a", 3_000),
)

GMDCSA24_ROBUSTNESS_S2_NIGHT = (
    SeedMediaSpec("gmdcsa24-s2-fall-13", "Subject 2/Fall/13.mp4", 1_617_503, "dfc8b89268975d8200782a2d396309dfe76c13b0", 8_000, 2_500, 8_000, "gmdcsa24-s2-fall-13-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s2-adl-10", "Subject 2/ADL/10.mp4", 7_729_870, "f50ccfcbaf330a5128247baa828c7464932accab", 7_000),
)

# The source table labels the fall from 1.8s to the nominal 7.0s clip end. The
# first exact-head run established that the exact MP4's decoded timestamp span
# is shorter than that nominal endpoint, so the evaluator correctly rejected
# the label before producing evidence. Keep the source interval documented, but
# evaluate the conservative 1.8s..6.5s subset: it lies wholly inside the source
# fall annotation, leaves 4.7s for the unchanged 3000ms rule, and cannot inflate
# a late match by extending beyond known decoded media.
GMDCSA24_ROBUSTNESS_S3_NIGHT_SW = (
    SeedMediaSpec("gmdcsa24-s3-fall-16", "Subject 3/Fall/16.mp4", 3_349_906, "a66bfe75bebe8643865bf685fb74e867f23ad6e9", 7_000, 1_800, 6_500, "gmdcsa24-s3-fall-16-labelled-falling"),
    SeedMediaSpec("gmdcsa24-s3-adl-06", "Subject 3/ADL/06.mp4", 9_084_601, "e66544079bc009369f1c4068b136de9f27606e2d", 8_000),
)
...
def _download_exact(url: str, target: Path, *, expected_size: int, opener: Callable[..., Any] = urllib.request.urlopen) -> None:
    if target.exists():
...
def _manifest(identities: dict[str, str]) -> dict[str, Any]:
    ...
    for spec in GMDCSA24_SEED:
        ...
        if spec.label_id is not None:
            labels.append({"start_timestamp_ms": spec.label_start_timestamp_ms, "end_timestamp_ms": spec.label_end_timestamp_ms, "label_id": spec.label_id})
        samples.append({
    ...
    return {"schema_version": 1, "artifact_root": "artifacts", "samples": samples, "config": {"max_samples": len(samples), "required_device": "CPU", "max_total_video_bytes": sum(item.size_bytes for item in GMDCSA24_SEED)}}
