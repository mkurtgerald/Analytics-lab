"""Bounded warm-runtime engineering evidence for the admitted OIDv4 Weapons path.

This lane re-verifies only the already-admitted standard OIDv4 frozen graph,
compiles it exactly once, and runs three sequential detections against one
deterministic generated in-memory uint8 tensor through the same Weapons runtime
instance. It records hosted-CI engineering timing/resource observations only;
it is not a product-performance, weapon-accuracy, or commercial-readiness claim.
"""
...
from .weapons_oid_ssd_smoke import _validate_detections
...
def run(work_dir: str | Path) -> dict[str, object]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="weapons-hardening-", dir=root) as temporary:
            ...
            detection_summaries: list[tuple[dict[str, object], ...]] = []
            for _ in range(_INFERENCE_RUNS):
                started = time.perf_counter()
                detections = runtime.detect(frame)
                inference_seconds.append(time.perf_counter() - started)
                detection_summaries.append(_validate_detections(detections))

        after = hashlib.sha256(frame.tobytes()).hexdigest()
        input_immutable = before == after
        if not input_immutable:
            raise RuntimeError("Weapons hardening mutated the synthetic input")
        if runtime.compile_count != 1:
            raise RuntimeError("Weapons runtime did not compile exactly once")
        if runtime.inference_count != _INFERENCE_RUNS:
            raise RuntimeError("Weapons runtime did not reuse the compiled detector")

        rss_after_kib = int(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)
        return {
            "evidence": "weapons-warm-runtime-hardening-v1",
            ...
            "detection_count_each_run": [len(items) for items in detection_summaries],
            "class_ids_each_run": [
                [item["class_id"] for item in items] for items in detection_summaries
            ],
            "phase_seconds": {
            ...
            "claim": (
                ...
                "performance, weapon-detection accuracy, or commercial readiness"
            ),
        }
    finally:
