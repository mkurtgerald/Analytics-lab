"""Run the retained person-down path on the second disjoint Figshare pair.

The source supplies clip-level Fall/ADL activity classes, not an independently
published frame-level event interval for these exact clips. The difficult ACT19
Laying clip is scored as a negative. The ACT4 Fall-on-the-back clip is retained
as diagnostic-only for perception, temporal continuity, candidate behavior and
resource cost; match/miss and alert latency are not fabricated from a directory
label. No detector, association, posture or temporal threshold is changed.
"""
...
from .figshare_generalization2_admission import admit_pinned_pair
...
_ACTIVITY = {
    ...
    "positive": {
        "activity_code": "ACT4",
        "activity_name": "Fall on the back",
...
def _sample_spec(item) -> ValidationSampleSpec:
    ...
    return ValidationSampleSpec(
        sample_id=f"figshare-g2-{activity['activity_code'].lower()}-{item.role}",
        site_id=f"figshare-28596332-location-{activity['location_id']}",
        camera_id=f"figshare-g2-{activity['activity_code'].lower()}-clip",
...
def run_figshare_generalization2_person_down_diagnostic(
    output_dir: str | Path,
    ...
    *,
    ...
    # Bind admitted media identity before spending runner time on model assets.
    ...
    return {
        "schema_version": 1,
        "diagnostic": "figshare_person_down_generalization_2",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = run_figshare_generalization2_person_down_diagnostic(args.output_dir, args.candidate_dir)
        encoded = json.dumps(payload, allow_nan=False, sort_keys=True, separators=(",", ":"))
        print(encoded)
        import os
        if os.getenv("GITHUB_ACTIONS") == "true":
            print("::notice title=Figshare second-pair person-down diagnostic::" + _github_escape(encoded))
    except (OSError, ValueError, TypeError, RuntimeError, OverflowError):
        print("Figshare second-pair person-down diagnostic rejected.", file=sys.stderr)
        return 2
    return 0
