"""Admit the predeclared fourth Figshare generalization pair through exact ranges.

The pair was selected from central-directory metadata in PR #55 before any
...
ADL_NEGATIVE = PinnedMember(
    role="negative",
    name="VideoDataset/ADL/SBJ_29_LOC1/ACT20_R_1/20240921145245.mp4",
    compressed_size=580_669,
    uncompressed_size=581_347,
    crc32=0x67DBCBE6,
    local_header_offset=1_118_347_300,
)
FALL_POSITIVE = PinnedMember(
    role="positive",
    name="VideoDataset/Fall/SBJ_07_LOC3/ACT6_R_1/20240914124502.mp4",
    compressed_size=518_570,
    uncompressed_size=519_269,
    crc32=0xC6D0FB47,
    local_header_offset=1_869_305_105,
)
...
def select_pinned_pair(members: tuple[ZipMember, ...]) -> tuple[tuple[PinnedMember, ZipMember], ...]:
    ...
    for expected in PINNED_PAIR:
        ...
        if len(matches) != 1:
            raise RuntimeError("fourth Figshare member identity is missing or changed")
        member = matches[0]
        if not member.is_bounded_video:
            raise RuntimeError("fourth Figshare member is outside the reviewed extraction envelope")
...
def admit_pinned_pair(output_dir: str | Path, *, opener: Any) -> tuple[AdmittedMember, ...]:
    ...
    for expected, member in selected:
        ...
        filename = "figshare-g4-adl-negative.mp4" if expected.role == "negative" else "figshare-g4-fall-positive.mp4"
        ...
        admitted.append(AdmittedMember(role=expected.role, name=member.name, compression_method=member.compression_method, compressed_size=member.compressed_size, uncompressed_size=member.uncompressed_size, crc32=member.crc32, sha256=sha256, local_path=target))
...
def evidence_document(admitted: tuple[AdmittedMember, ...]) -> dict[str, Any]:
    ...
    return {
        "pair_id": PAIR_ID,
        "source": {"source_id": FIGSHARE_FALL_2017.source_id, "title": FIGSHARE_FALL_2017.title, "version": FIGSHARE_FALL_2017.version, "license_id": FIGSHARE_FALL_2017.license_id, "provenance_ref": FIGSHARE_FALL_2017.provenance_ref, "attribution_required": FIGSHARE_FALL_2017.attribution_required},
        "members": [{"role": item.role, "name": item.name, "compression_method": item.compression_method, "compressed_size": item.compressed_size, "uncompressed_size": item.uncompressed_size, "crc32": f"{item.crc32:08x}", "sha256": item.sha256} for item in admitted],
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("figshare_generalization4_member_admission=" + payload)
    if os.getenv("GITHUB_ACTIONS") == "true":
        print("::notice title=Figshare fourth-pair admission::" + _github_escape(payload))
    return 0
