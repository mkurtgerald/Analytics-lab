...
ADL_NEGATIVE = PinnedMember(
    role="negative",
    name="VideoDataset/ADL/SBJ_26_LOC1/ACT18_R_1/20240921134706.mp4",
    compressed_size=598_772,
    uncompressed_size=599_429,
    crc32=0xA199B8B5,
    local_header_offset=1_280_237_845,
)
FALL_POSITIVE = PinnedMember(
    role="positive",
    name="VideoDataset/Fall/SBJ_08_LOC3/ACT3_R_1/20240914130429.mp4",
    compressed_size=517_803,
    uncompressed_size=518_474,
    crc32=0xF9D06B88,
    local_header_offset=2_174_699_626,
)
...
def select_pinned_pair(members: tuple[ZipMember, ...]) -> tuple[tuple[PinnedMember, ZipMember], ...]:
    ...
    for expected in PINNED_PAIR:
        ...
        if len(matches) != 1:
            raise RuntimeError("fifth Figshare member identity is missing or changed")
        member = matches[0]
        if not member.is_bounded_video:
            raise RuntimeError("fifth Figshare member is outside the reviewed extraction envelope")
...
def admit_pinned_pair(output_dir: str | Path, *, opener: Any) -> tuple[AdmittedMember, ...]:
    ...
    for expected, member in selected:
        ...
        filename = (
            ...
            else "figshare-g5-fall-positive.mp4"
        )
...
def evidence_document(admitted: tuple[AdmittedMember, ...]) -> dict[str, Any]:
    if (
        ...
        or len(admitted) != 2
        or any(not isinstance(item, AdmittedMember) for item in admitted)
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("figshare_generalization5_member_admission=" + payload)
    if os.getenv("GITHUB_ACTIONS") == "true":
        print("::notice title=Figshare fifth-pair admission::" + _github_escape(payload))
    return 0
