"""Bounded public-domain vehicle-scene evidence for LPR detector localization.

This module admits one real vehicle photograph whose copyright holder released it
to the public domain. The first head is admission-only and discovers immutable
hashes without inference. Only after exact SHA-1 and SHA-256 values are pinned
may the already-reviewed OMZ 0106 plate detector run on CPU.

A single photograph is an engineering smoke diagnostic, never commercial
accuracy/generalization evidence.
"""
...
_RIGHTS_PAGE = (
    ...
    "File:PR_China_license_plate_Beijing_%E4%BA%ACB%E2%80%A2K0074_Taxi.jpg"
    "&oldid=1109767728"
)
_SOURCE_URL = (
...
def validate_source_bytes(
    ...
    *,
    ...
    if required_sha1 is not None:
        if not isinstance(required_sha1, str) or len(required_sha1) != 40:
            raise ValueError("required SHA-1 must be lowercase hex")
...
def run() -> dict[str, object]:
    ...
    common: dict[str, object] = {
        "evidence_scope": "single_public_domain_vehicle_scene_smoke_not_commercial_accuracy",
    ...
    try:
        ...
        raise RuntimeError("OpenCV and NumPy are required for LPR vehicle-scene evidence") from exc
    ...
    with tempfile.TemporaryDirectory(prefix="analytics-lpr-scene-") as directory:
        root = Path(directory)
