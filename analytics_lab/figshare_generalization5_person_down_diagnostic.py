"""Run the unchanged retained person-down path on Figshare pair five.

The source supplies clip-level Fall/ADL activity classes, not an independently
published frame-level event interval for these exact clips. ACT18 Picking-up is
scored as a difficult negative. ACT3 Fall-on-the-front is diagnostic-only for
perception, temporal continuity, candidate behavior and resource cost; match/
miss and alert latency are not fabricated from a directory label. No detector,
association, posture or temporal threshold changes.
"""
...
from .figshare_generalization5_admission import EXPECTED_SHA256, admit_pinned_pair
...
_ACTIVITY = {
    ...
    "positive": {
        "activity_code": "ACT3",
        "activity_name": "Fall on the front",
...
def run_figshare_generalization5_person_down_diagnostic(
    output_dir: str | Path,
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "diagnostic": "figshare_person_down_generalization_5",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = run_figshare_generalization5_person_down_diagnostic(args.output_dir, args.candidate_dir)
        encoded = json.dumps(payload, allow_nan=False, sort_keys=True, separators=(",", ":"))
        print(encoded)
        import os
        if os.getenv("GITHUB_ACTIONS") == "true":
            print("::notice title=Figshare fifth-pair person-down diagnostic::" + _github_escape(encoded))
    except (OSError, ValueError, TypeError, RuntimeError, OverflowError):
        print("Figshare fifth-pair person-down diagnostic rejected.", file=sys.stderr)
        return 2
    return 0
