...
### Rights-cleared real-person source admission — pinning head
...
Exact-head discovery run #266 passed the repository guardrails, Linux regression, the bounded source-admission step, Windows regression and the aggregate quality gate. The source was streamed only in memory, never decoded, and produced SHA-256 `7356daa8fd4ad53b946ce0036f06b014431dc89b7ae29ecd8ef18fc54edce6b5`. The discovery emitted no face count, did not install or execute a model/runtime, retained/uploaded no media, and made no detection-accuracy or commercial-readiness claim.

The current PR #108 head hard-pins that exact SHA-256 and must reverify the same size/SHA-1/SHA-256 under the same bounded no-decode/no-inference lane. Only after this exact pinning head is green and merged is the source admitted for a **separate** one-attempt face-detection -> automatic-blur measurement. That later measurement must use the already-admitted standard OIDv4 model, fixed pre-measurement threshold `0.50`, preserve all misses and false positives, and perform no tuning before the first result.
