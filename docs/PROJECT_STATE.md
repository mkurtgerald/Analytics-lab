...
The current PR head hard-pins those archive/member identities, explicitly requires the frozen graph, SavedModel graph and pipeline config, and fails closed if a label-map member unexpectedly appears. Artifact admission is valid only when exact-head hosted CI re-downloads and re-verifies those identities unchanged.

### Next executable face gate after admission
After the standard donor's exact-head artifact admission is green and merged, the next work item is the smallest runtime-construction path using only already-reviewed runtime families. Attempt exact standard frozen-graph construction with OpenVINO `2026.3.1` and/or OpenCV `4.12.0`, one bounded first attempt each only when justified. Construction success is compatibility evidence only, not accuracy.

If a standard graph constructs, immediately add a model-specific adapter that filters only OIDv4 class `502` (`Human face`) into the existing normalized `FaceDetection` contract and default-on blur pipeline. The first execution after construction must be a no-media synthetic tensor smoke. Only after that is green may a separately rights-cleared real-person source be admitted for one untouched face-detection -> automatic-blur measurement. Preserve misses and false positives and do not tune before that first measurement.
