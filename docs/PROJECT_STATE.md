...
The current candidate increment adds `analytics_lab.openvino_omz`, an optional OpenVINO 2026.3.1 adapter boundary. It verifies the exact four local model artifacts before runtime creation, parses the reviewed SSD detector output, crops detected people, runs the reviewed pose model, extracts only the four shoulder/hip heatmap peaks required by the posture classifier, and emits `PoseCandidate` values for the existing temporary tracker/posture pipeline. The adapter has no downloader, no URL/device capture, no identity/ReID, and no production-release path.

The keypoint channel IDs and quarter-pixel peak refinement are adapted from the Apache-2.0 Open Model Zoo OpenPose decoder at the same pinned commit. The full PAF grouping decoder is intentionally not copied: the present detector-isolated crop method is a small integration baseline that must earn its accuracy through labeled-video measurement.
...
For the OpenVINO adapter candidate, Python 3 local focused tests passed 10/10 before any repository mutation. Tests cover detector threshold/class/sentinel parsing, normalized-box scaling and caps, malformed/non-finite outputs, required pose heatmap channels/geometry, runtime/config bounds, artifact-verification-before-runtime construction, runtime-version pin enforcement, and frame/image metadata validation. The focused harness uses an injected fake runtime; OpenVINO itself and model binaries were not downloaded or executed in this run. Exact-head GitHub CI is required before integration.

No detector, pose, fall, person-down or tracking accuracy claim is made by these structural tests.
...
After exact-head CI passes this candidate, provision the already recorded model artifacts plus OpenVINO 2026.3.1 outside the public repository and perform one bounded CPU runtime execution. First prove verified artifacts -> actual detector output -> actual pose heatmaps -> `PoseCandidate` without retaining media. Then run one authorized labeled positive clip plus normal negatives through video -> OpenVINO backend -> temporary tracking -> posture -> temporal event and record false alerts, misses, latency, hardware settings and exact artifact/runtime identities. If the detector-isolated four-keypoint baseline underperforms, compare it against the pinned full OpenPose decoder before adding another framework.
