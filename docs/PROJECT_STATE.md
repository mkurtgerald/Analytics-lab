...
### First exact-head 0006 result
Head `cb5b7d063f72f1a4fcaa2899c127c63c184c3185`, workflow run 100, attempt 1 passed policy classification, all 43 guardrail regressions, all 227 synthetic tests (1 optional OpenCV skip), synthetic replay, pinned runtime provisioning and the retained Subject-4 baseline. The baseline reproduced 0 positive candidates / 1 miss and 0 negative candidates / 0 false alerts with 55 qualified-down observations and a 1312 ms longest run. The candidate then failed closed at `candidate_decode`; Linux failed, Windows was correctly skipped and Analytics quality failed. This was not a `0006` performance result.

The deterministic cause was local: `AssociativeEmbeddingDecoder.__call__` hard-coded the prior `0005` 144x144 heatmap/tag shapes even though the pinned upstream decoder is spatial-size agnostic. The current correction changes only that adapter contract: batch=1, 17 keypoints, positive square H/W, matching embedding H/W and tag-size=1 remain enforced; reviewed 176x176 output is now admitted. A focused regression covers 176x176 acceptance plus mismatched-spatial rejection. No detector, threshold, association, posture, 3000 ms persistence, 750 ms unknown-gap, media or evaluator change is made.
...
## Efficiency ledger
One worker, one acceptance-moving item, one open implementation/evidence PR (#42). No paid/GPU/self-hosted resource, home/customer media, training, production promotion, release action or commercial accuracy claim is authorized. The current fresh session begins from the completed deterministic failure on head `cb5b7d063f72f1a4fcaa2899c127c63c184c3185`; active runs for that head are 0, unchanged retries are 0, and this bounded adapter correction is the first CI-triggering mutation of the fresh session. If the corrected A/B fails its retention rule, close PR #42 unmerged. If it passes, merge only after exact-head Linux, Windows and Analytics quality are green on the unchanged tested base.
