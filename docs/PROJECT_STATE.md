...
Current acceptance is artifact admission only: download the exact Google archive in bounded hosted CI, compute immutable archive size/SHA-256 and inspect model/config members in memory. No inference or media occurs until those hashes are pinned. Run #215 preserved a pre-admission transport failure: the legacy `download.tensorflow.org` hostname presented a certificate whose hostname did not match on the hosted runner. No payload or model bytes were accepted. The repaired head uses Google's direct TLS-valid GCS bucket path `https://storage.googleapis.com/download.tensorflow.org/models/object_detection/facessd_mobilenet_v2_quantized_320x320_open_image_v4.tar.gz` for the same archive; certificate verification remains enabled and no HTTP downgrade is permitted. Run #216 reached the direct Google bucket with TLS intact but rejected the archive before download because its declared size exceeded the preregistered 32 MiB ceiling. No model payload was accepted. The next head performs a HEAD-only metadata admission first; if the declared size is above the old ceiling it emits that exact byte count and downloads zero payload bytes, allowing the subsequent head to raise the bound only to the observed Google object size before hashing the archive. Run #217 passed Linux/Windows/final gating and reported Google's exact declared archive size as 130,655,026 bytes with `payload_downloaded=false`. The current head pins both the maximum bound and expected size to exactly 130,655,026 bytes, then downloads the archive once to compute SHA-256 and inspect recognized model/config members in memory. The SHA-256 remains intentionally unpinned until that first full-payload witness exists.
