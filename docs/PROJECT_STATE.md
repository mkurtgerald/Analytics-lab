...
### Preserved converter decision — TensorFlow TOCO source build not admitted
The post-PR #103 fallback reviewed TensorFlow 2.21.0 source target `//tensorflow/lite/toco:toco` at exact commit `a481b10260dfdf833a1b16007eead49c1d7febf3`. The source pins Bazel 7.7.0 and the public TOCO binary links TensorFlow core/lib/ops/protos, TFLite import/export, protobuf, Abseil and the full TOCO graph-transformation stack. Under this repository's five-minute hosted-job ceiling and exact dependency/toolchain rules, that is not a bounded standalone converter path. Do not spend a CI build attempting it; the hypothesis is closed before execution rather than consuming the day on an oversized source build.

### Current face acceptance item — already-admitted OpenVINO TensorFlow frontend
A smaller path exists inside an already-reviewed dependency family: OpenVINO Runtime Python API `2026.3.1`, exact source commit `759c5a6ab8c066af5f4bc5ebd04643706012a37d`, Apache-2.0. The pinned source exports `openvino.convert_model` and contains a native TensorFlow frontend with GraphDef conversion coverage. This repository already admits OpenVINO 2026.3.1 for bounded hosted CPU evidence, so this path adds no new inference framework family.

The first FaceSSD/OpenVINO head must reverify the exact Google archive/member identities, materialize only pinned `tflite_graph.pb` under `RUNNER_TEMP`, install only `openvino==2026.3.1`, call `openvino.convert_model` on the frozen GraphDef, then—only if conversion succeeds—compile the in-memory result with the OpenVINO CPU plugin. It must not call inference, provide an input tensor, serialize/retain the converted model, process media, install TensorFlow/TFLite, or claim detection accuracy. Report only conversion/compile success plus bounded input/output metadata. A conversion or CPU compile failure is a valid negative classification and closes this path without retry/tuning.

Run #256 preserved the first exact result: full-graph `openvino.convert_model` returned `OpConversionFailure`; no CPU compile, inference or media occurred. Authoritative TensorFlow Models source at `8b12ae202a3ccf8f965c730a4e7617204e32000b` documents that the exported graph appends a custom `TFLite_Detection_PostProcess` node whose exact inputs are `raw_outputs/box_encodings`, `raw_outputs/class_predictions`, and `anchors`. The one diagnosed follow-up therefore does not change weights or tune the detector: it asks the same OpenVINO frontend to preserve exactly those three pre-postprocess tensors as graph outputs. If that cut graph converts and compiles, the admitted detector core can remain while K5 implements the documented SSD box decode/NMS behind the normalized FaceDetection boundary; if it fails, the OpenVINO path is closed. This is the second and final CI-triggering action for this execution session.
