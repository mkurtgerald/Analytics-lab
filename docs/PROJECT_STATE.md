...
The smallest runnable LPR/OCR engineering baseline is therefore frozen at the already-verified Tesseract 5.5.3 path plus the existing replaceable detector/proposal boundaries. Remaining non-baseline gates are broader independently labeled rights-cleared held-out evidence, a stronger OCR donor only if code/weight/data rights close cleanly, native dependency/redistribution notices, representative soak/performance work, and explicit commercial-release approval. PR #111 exact-head run #278 and post-merge main run #279 are green. The single active implementation lane is now Weapons. PR #112 exact-head run #280 and post-merge main run #281 are green, closing the first fixed seven-class OIDv4 weapon allowlist/parser baseline behind the existing neutral `DetectionCandidate` boundary. PR #113 exact-head run #282 and post-merge main run #283 are also green, closing the compile-once warm-reuse lifecycle around the same admitted graph/runtime. PR #114 exact-head run #285 and post-merge main run #286 are green, closing the hash-locked exact-model Weapons execution-compatibility gate: the admitted archive/graph reverified, the model compiled once, exactly one generated 300x300 uint8 inference ran at the frozen 0.50 threshold, the input remained immutable, and no media/model/pixel artifact was retained. The generated blank tensor emitted zero allowlisted detections; this is preserved as a synthetic engineering observation only. The active next gate is source admission for two individually rights-cleared CC0 real images—a Winchester Model 1895 rifle positive candidate and a chair non-weapon negative candidate. First-head run #287 passed Linux, Windows and the Analytics quality gate and discovered exact SHA-256 identities without decode or inference: rifle `4c58e411b9e3e7348420915dcb32be0e6dcd0132f6120a6049af36791605c0f2`; chair `73db796087c551375d7d537ca78c81ede7b106cd8d25e1fad729a29a7b59483c`. Pinning head run #288 passed Linux, Windows and the Analytics quality gate, PR #115 merged as `029fbbe71e92265d957285b3dfd08f6deeaf3d2d`, and post-merge run #289 passed. The source-admission gate is therefore closed green. The next prepared gate is one untouched two-source Weapons measurement: stream/reverify the admitted graph, compile once, decode each pinned CC0 source in memory with EXIF auto-orientation disabled, execute exactly one frozen-0.50 inference per source, preserve every allowlisted detection, verify input immutability, and keep misses/false positives unscored because source roles are selection metadata rather than independent detector ground truth. PR #116 exact-head run #290 and post-merge main run #291 are green, closing the first untouched two-source real Weapons engineering observation. At the frozen 0.50 threshold, the admitted OIDv4 model emitted zero allowlisted detections on both the Winchester Model 1895 rifle-selected CC0 source and the Chair.JPG non-weapon CC0 source; both inputs remained immutable, compile_count=1 and inference_count=2, and no media/model artifact was retained. Because independently authored detector ground truth is unavailable, misses and false positives remain unscored and this is not an accuracy claim. The zero output on the rifle-selected source is preserved as a material engineering signal and may justify evaluating a stronger separately admitted detector/model path after the current runtime-hardening gate. The active next gate is bounded Weapons warm-runtime/resource hardening using one deterministic generated tensor, one compile and three sequential detections through the same runtime. Tracking remains frozen at its independent-label blocker.
