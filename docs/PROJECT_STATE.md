...
`analytics_lab.evaluation` provides deterministic one-to-one matching of person-down candidate evidence windows against bounded labeled person-down intervals. It reports matched alerts, misses, false alerts, precision/recall when denominators exist, false alerts per camera-hour, and alert delay. Repeated candidate alerts cannot inflate recall because each label and event can match at most once. Inputs are bounded and malformed/out-of-interval events fail closed. These metrics measure candidate behavior only and do not imply a fall, injury, cause, fault or intent.

The current candidate extends that evaluation boundary to multi-sample/site measurement. Each evaluation sample binds an opaque sample/site/camera identity to a concrete time interval and its single-video result. Aggregate precision/recall are calculated from raw counts rather than averaging percentages, exact matched-event delays are combined for a true aggregate median, and per-site/per-camera summaries are returned. Duplicate sample IDs and overlapping intervals for the same camera are rejected so repeated footage cannot silently inflate camera-hours, recall, or false-alert denominators.

## Observed validation
Baseline integration commit `659ea73a9149d8bb739f019f8784aafa81870ecb` had 35 local unit tests. Efficiency controls were integrated at `795668874ab2598409206abde12f1f3715828a2c`. Local-video bridge commit `7f72435937eca7363bc35b5788b878192c607843` passed exact-head PR CI and first-attempt post-merge CI. Pose/artifact commit `c5aa7c05980f7145e8bf5cb379ff5f5da5133796` passed exact-head PR CI and first-attempt post-merge Linux/Windows/quality-gate CI. OpenVINO adapter commit `7c56e56888560e6d9c7e0399ab74d958af117516` passed exact-head PR CI and first-attempt post-merge Linux/Windows/quality-gate CI. Pipeline composition commit `b8937687a8a0586066ce7d0b07be9da963a5f6e5` passed its post-merge Linux, Windows and Analytics quality gate checks on the first attempt. Evaluation commit `104f832364e4e759ecbdabbd45667e2d4ef3f141` passed its exact-head and post-merge Linux, Windows and Analytics quality gate checks on the first attempt.

For the multi-sample evaluation candidate, ten focused local unittest cases passed, including the original single-video behavior plus micro-aggregation, site/camera breakdowns, duplicate sample rejection, same-camera overlap rejection and sample-duration binding. An additional 500 deterministic randomized aggregate cases preserved the count invariants `matched + false = candidates` and `matched + missed = positives`. These tests do not execute OpenVINO or media; exact-head GitHub CI remains required before integration.

No detector, pose, fall, person-down or tracking accuracy claim is made by structural, injected-runtime, synthetic evaluation or aggregation tests.
...
Provision the already recorded model artifacts plus OpenVINO 2026.3.1 outside the public repository and perform one bounded CPU runtime execution through `run_frame_source_openvino_omz` or `run_local_video_openvino_omz`. The current worker environment does not have OpenVINO installed, and no large runtime download is authorized by the efficiency policy, so no runtime package was fetched in this increment. First prove verified artifacts -> actual detector output -> actual pose heatmaps -> temporary tracking/posture -> candidate event without retaining media. Then feed multiple authorized labeled positive and negative samples into `evaluate_person_down_candidates`, bind each result with `EvaluationSample`, and aggregate with `aggregate_person_down_evaluations` to measure false alerts, misses, precision/recall where defined, alert delay and per-site/per-camera behavior. Record latency, hardware settings and exact artifact/runtime identities separately. If the detector-isolated four-keypoint baseline underperforms, compare it against the pinned full OpenPose decoder before adding another framework.
