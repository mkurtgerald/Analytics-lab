...
Runs #269 and #270 both passed the normal Linux regressions and reached the dedicated measurement step, then were cancelled by the repository's existing five-minute Linux job ceiling before any face result was emitted. This is a runtime-budget failure, not a detector miss. The diagnosed cause is redundant execution-time re-admission: the measurement downloaded and hashed the entire 158,851,107-byte donor archive even though PR #105/run #260 already immutably admitted that archive and independently pinned the exact 66,606,111-byte frozen graph. The repaired head keeps the canonical Google archive URL and verifies its admitted Content-Length, streams only until the exact pinned `frozen_inference_graph.pb` member is reached, verifies that member's size/SHA-256, and stops consuming the archive. It does not weaken model identity, source identity, the fixed 0.50 threshold, media controls, or the five-minute ceiling. Phase timing is emitted for graph stream/verification, source stream/verification, image decode, model convert/compile, and the single inference+blur so any future budget regression is diagnosable rather than opaque.
