...
### Preserved FaceSSD runtime classification — OpenCV paths closed
PR #88 merged at `cf28001e26b44309c7e7cbf28735ed915b3cba1e`; exact-head run #224 and post-merge run #225 passed. OpenCV's exact 4.12.0 SSD graph helper, pinned at `opencv/opencv@49486f61fb25722cbcf586b7f4320921d46fb38e`, exited 1 before producing a derived pbtxt; `rewrite_succeeded=false` and `rewritten_runtime_compatible=false`. Together with PR #87's direct-import result, this closes the planned OpenCV-only FaceSSD execution routes. No inference or media occurred.

### Current face acceptance item — LiteRT 0.4.0 build-time converter admission
The measured OpenCV incompatibility now justifies one second framework for conversion only. Prefer Google's stable LiteRT converter over the much larger TensorFlow CPU wheel. Exact source baseline `google-ai-edge/LiteRT@7d683c3c1104c29a4777d7047cff2fbe92bacce3` declares `LITERT_CONVERTER_VERSION = "0.4.0"` and Apache-2.0 licensing; the same source still implements `TFLiteConverter.from_frozen_graph`. The CPython 3.11 Linux wheel is pinned at SHA-256 `77827e51886bca2fea2e56ce98e1fcc66f7985260c9b6bb6786ede1374638b51`.

The first head is dependency admission only: hosted Linux uses pip download with binary wheels only, beneath `RUNNER_TEMP`, for `litert-converter==0.4.0`. It installs/imports nothing, hashes every resolved wheel, reads only bounded METADATA identity/license/Requires-Dist fields, caps the complete closure at 180 MiB and deletes it before exit. This closes exact build-time dependency identity before any conversion.

If that closure is permissively licensed and bounded, the next head pins every wheel version/hash and performs one frozen conversion attempt against the exact admitted FaceSSD `tflite_graph.pb`, using the donor's exact `pipeline.config` to bind input geometry and Google's documented frozen-graph input/output names. It must create the derived TFLite only under `RUNNER_TEMP`, hash/bound it, run no inference/media, upload/retain nothing, and preserve the first result without tuning.
