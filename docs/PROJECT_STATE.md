...
### Current face acceptance item — direct pinned-OpenCV runtime classification
The next bounded step tests the smallest zero-new-framework execution path before any converter or TensorFlow/TFLite runtime is considered. On a dedicated `evidence/face-donor-facessd-runtime-*` hosted Linux branch, the repository must first re-verify the exact admitted archive SHA-256 and all seven regular member identities. It may then materialize only the pinned `tflite_graph.pb` and, if needed, the pinned shipped `tflite_graph.pbtxt` beneath `RUNNER_TEMP`, install only the already-reviewed `opencv-python-headless==4.12.0.88`, and call OpenCV's TensorFlow graph importer. It must perform no `forward`, inference, media decode, real-person processing, conversion, model upload or retained artifact.

The result is a runtime-import classification, not an accuracy measurement. Successful import proves only that the exact graph can be constructed by the pinned OpenCV runtime; failure closes this direct path and does not authorize a second framework, converter, alternate weight, or unreviewed dependency. The first head must preserve whichever result occurs without tuning the donor or weakening import errors. Any next execution path requires a separately reviewed dependency/artifact boundary.
